...
def test_user_autocomplete():
    ...
    another_beneficiary = JobSeekerFactory(
        first_name="gps another beneficiary first_name"
    )

    FollowUpGroupFactory(
        beneficiary=beneficiary, memberships=4, memberships__member=member
    )
    ...
    assert (
        User.objects.autocomplete("gps member", kind=UserKind.PRESCRIBER).count() == 1
    )
...
def test_join_group_as_job_seeker(is_referent, client, snapshot):
    ...
    membership = (
        FollowUpGroupMembership.objects.filter(member=prescriber)
        .filter(follow_up_group=follow_up_group)
        .first()
    )
...
def test_navigation(snapshot, client):
    member = PrescriberFactory(first_name="gps member first_name")
    member_first_beneficiary = JobSeekerFactory(
        first_name="gps first beneficiary first_name"
    )
    member_second_beneficiary = JobSeekerFactory(
        first_name="gps second beneficiary first_name"
    )

    FollowUpGroupFactory(
        beneficiary=member_first_beneficiary, memberships=4, memberships__member=member
    )
    FollowUpGroupFactory(
        beneficiary=member_second_beneficiary, memberships=2, memberships__member=member
    )
