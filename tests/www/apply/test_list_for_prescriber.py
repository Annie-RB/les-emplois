...
def test_list_for_prescriptions(client):
    """
    ...
    response = client.get(reverse("apply:list_for_prescriptions"))
    assert_previous_step(response, reverse("dashboard:index"))
    # Has link to export with back_url set
    exports_link = unquote(
        add_url_params(
            reverse("apply:list_for_prescriptions_exports"), {"back_url": reverse("apply:list_for_prescriptions")}
        )
    )
    ...
    job_application_link = unquote(
        add_url_params(
            ...
            {"back_url": reverse("apply:list_for_prescriptions")},
        )
    )
    ...
    company_link = unquote(
        add_url_params(
            job_application.to_company.get_card_url(), {"back_url": reverse("apply:list_for_prescriptions")}
        )
    )
...
def test_list_for_prescriptions_as_unauthorized_prescriber(client):
...
def test_list_for_prescriptions_filtered_by_state(client):
    """
    ...
    response = client.get(reverse("apply:list_for_prescriptions"), {"states": [job_application.state]})
...
def test_list_for_prescriptions_filtered_by_sender(client):
    ...
    response = client.get(reverse("apply:list_for_prescriptions"), {"senders": another_prescriber.pk})
    applications = response.context["job_applications_page"].object_list
    assert len(applications) == 1
    assert applications[0].sender.id == another_prescriber.pk


def test_list_for_prescriptions_filtered_by_job_seeker(client):
    ...
    response = client.get(reverse("apply:list_for_prescriptions"), {"job_seekers": [job_seeker.pk]})
    applications = response.context["job_applications_page"].object_list
    assert len(applications) == 1
...
def test_list_for_prescriptions_filtered_by_company(client):
    prescriber = PrescriberFactory()
    job_application, *others = JobApplicationFactory.create_batch(3, sender=prescriber)
    client.force_login(prescriber)

    response = client.get(reverse("apply:list_for_prescriptions"), {"to_companies": [job_application.to_company.pk]})
    applications = response.context["job_applications_page"].object_list
    assert len(applications) == 1
...
def test_list_for_prescriptions_filters(client, snapshot):
...
def test_list_for_prescriptions_htmx_filters(client):
    prescriber = PrescriberFactory()
    ...
    response = client.get(
        reverse("apply:list_for_prescriptions"),
        {"states": ["refused"]},
        headers={"HX-Request": "true"},
    )
    update_page_with_htmx(page, "#asideFiltersCollapse > form", response)

    response = client.get(reverse("apply:list_for_prescriptions"), {"states": ["refused"]})
    fresh_page = parse_response_to_soup(response, selector="#main")
    assertSoupEqual(page, fresh_page)


def test_list_for_prescriptions_exports_without_organization(client):
    client.force_login(PrescriberFactory())

    response = client.get(reverse("apply:list_for_prescriptions_exports"))
    assert_previous_step(response, reverse("dashboard:index"))
    assertNotContains(response, BESOIN_DUN_CHIFFRE)


def test_list_for_prescriptions_exports_with_organization(client):
...
def test_list_for_prescriptions_exports_as_pole_emploi_prescriber(client, snapshot):
...
def test_list_for_prescriptions_exports_as_employer(client):
    job_application = JobApplicationFactory()
...
def test_list_for_prescriptions_exports_back_to_list(client):
    client.force_login(PrescriberFactory())
    ...
    assert_previous_step(response, reverse("apply:list_for_prescriptions"), back_to_list=True)
    assertNotContains(response, BESOIN_DUN_CHIFFRE)


def test_list_for_prescriptions_exports_download(client):
    job_application = JobApplicationFactory()
    client.force_login(job_application.sender)

    response = client.get(reverse("apply:list_for_prescriptions_exports_download"))
    assert 200 == response.status_code
    assert "spreadsheetml" in response.get("Content-Type")


def test_list_for_prescriptions_exports_download_as_employer(client):
    job_application = JobApplicationFactory()
    client.force_login(job_application.to_company.members.get())
...
def test_list_for_prescriptions_exports_download_by_month(client):
    job_application = JobApplicationFactory()
    client.force_login(job_application.sender)

    response = client.get(
        reverse(
            "apply:list_for_prescriptions_exports_download",
